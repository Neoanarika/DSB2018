...
def compute_precision_for_box(box, truth_box, truth_label, thresholds=[0.5]):
    ...
    for threshold in thresholds:
        truth_result = np.ones(num_truth_box, np.int32)
        result       = np.ones(num_box, np.int32)

        # truth_result
        truth_result[...] = INVALID
        truth_result[(max_overlap <threshold) & (truth_label>0)] = MISS
        truth_result[(max_overlap>=threshold) & (truth_label>0)] = HIT

        # result
        result[...] = FP
        result[argmax_overlap[truth_result==HIT]] = TP

        index = np.where(result==FP)[0]
        if len(index)>0:
            index = index[np.where(invalid_valid_overlap[index]>threshold)[0]]
            result[index] = INVALID


        ##-----------------------------
        num_truth = (truth_result!=INVALID).sum()
        num_hit   = (truth_result==HIT    ).sum()
        num_miss  = (truth_result==MISS   ).sum()
        recall    = num_hit / num_truth

        num_tp = (result==TP).sum()
        num_fp = (result==FP).sum()
        precision = num_tp / max(num_tp + num_fp + num_miss, 1e-12)
    ...
    return  precisions, recalls, results, truth_results
