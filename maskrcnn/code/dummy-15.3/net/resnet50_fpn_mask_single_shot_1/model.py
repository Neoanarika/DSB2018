...
from net.resnet50_fpn_mask_single_shot_1.configuration import *

from net.resnet50_fpn_mask_single_shot_1.layer.rpn_nms    import *
from net.resnet50_fpn_mask_single_shot_1.layer.rpn_target import *
from net.resnet50_fpn_mask_single_shot_1.layer.rpn_loss   import *
...
class FeatureNet(nn.Module):
    ...
    def __init__(self, cfg, in_channels, out_channels = 1024):
        super(FeatureNet, self).__init__()
        self.cfg=cfg

        # bottom-top
        self.layer_c0 = make_layer_c0(in_channels, 32)

        self.layer_c1 = make_layer_c(   32,  32,   64, num_blocks=2, stride=1)
        self.layer_c2 = make_layer_c(   64,  32,  128, num_blocks=2, stride=2)
        self.layer_c3 = make_layer_c(  128,  64,  256, num_blocks=3, stride=2)
        self.layer_c4 = make_layer_c(  256, 128,  512, num_blocks=4, stride=2)
        self.layer_c5 = make_layer_c(  512, 256, 1024, num_blocks=3, stride=2)

        # top-down
        self.layer_p5 = nn.Conv2d   (1024, out_channels//4, kernel_size=1, stride=1, padding=0)
        self.layer_p4 = LateralBlock( 512, out_channels//4, out_channels//4)
        self.layer_p3 = LateralBlock( 256, out_channels//4, out_channels//4)
        self.layer_p2 = LateralBlock( 128, out_channels//4, out_channels//4)
        self.layer_p1 = LateralBlock(  64, out_channels//4, out_channels//4)
    ...
    def forward(self, x):
        ...
        # merge
        p1 = p1
        p2 = F.upsample(p2, scale_factor=2,mode='bilinear')
        p3 = F.upsample(p3, scale_factor=4,mode='bilinear')
        p4 = F.upsample(p4, scale_factor=8,mode='bilinear')
...
class RpnHead(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.logit = nn.Conv2d(256, self.num_bases*self.num_classes,   kernel_size=3, padding=1)
        self.delta = nn.Conv2d(256, self.num_bases*self.num_classes*4, kernel_size=3, padding=1)
    ...
    def forward(self, f):
        ...
        f = F.relu(self.conv(f))
        logits = self.logit(f)
        deltas = self.delta(f)

        logits_flat = logits.permute(0, 2, 3, 1).contiguous().view(batch_size, -1, self.num_classes)
        deltas_flat = deltas.permute(0, 2, 3, 1).contiguous().view(batch_size, -1, self.num_classes, 4)
        probs_flat  = F.softmax(logits_flat, 2)  #softmax  #sigmoid
...
class CropRoi(nn.Module):
    ...
    def forward(self, f, proposals):

        rois  = proposals[:,0:5]
        crops = self.crop(f, rois)
...
class MaskSingleShotNet(nn.Module):
    ...
    def loss(self, inputs, truth_boxes, truth_labels, truth_instances):
        ...
        self.mask_cls_loss = Variable(torch.zeros((1))).cuda().sum() #??? how to create scalar in torch 0.40???
...
def run_check_rpn_head():
    ...
    logits_flat, probs_flat, deltas_flat = rpn_head(f)
