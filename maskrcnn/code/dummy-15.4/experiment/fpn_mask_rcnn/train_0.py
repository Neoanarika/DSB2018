...
from net.resnet50_fpn_mask_mask_rcnn.draw  import *
from net.resnet50_fpn_mask_mask_rcnn.model import *
# -------------------------------------------------------------------------------------
...
def run_train():
    ...
    out_dir  = RESULTS_DIR + '/mask-rcnn-dummy-12'
    initial_checkpoint = \
        None #RESULTS_DIR + '/mask-single-shot-dummy-12/checkpoint/00001400_model.pth'
        ##None #
    ...
    cfg = Configuration()
    net = MaskSingleShotNet(cfg).cuda()
        ...
        for inputs, truth_boxes, truth_labels, truth_instances, metas, indices in train_loader:
            ...
            if 1:
                ...
                for b in range(batch_size):
                    ...
                    all2 = draw_multi_rpn_delta(cfg, image, rpn_prob_flat, rpn_delta_flat, window)
                    all3 = draw_multi_rpn_proposal(cfg, image, proposal)
                    all4 = draw_truth_box(cfg, image, truth_box, truth_label)
                    all5 = draw_multi_rpn_metric(cfg, image, proposal, truth_box, truth_label)
                    all6 = draw_mask_metric(cfg, image, mask, truth_box, truth_label, truth_instance)

                    image_show('rpn_prob',all1,1)
                    image_show('rpn_delta',all2,1)
                    image_show('rpn_proposal',all3,1)
                    image_show('truth_box',all4,1)
                    image_show('precision',all5,1)
                    image_show('iou',all6,1)
                    cv2.waitKey(1)
