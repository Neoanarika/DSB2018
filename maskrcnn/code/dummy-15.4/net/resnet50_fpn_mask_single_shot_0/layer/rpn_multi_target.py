...
def draw_rpn_target_label(cfg, image, window, label, label_assign, label_weight):
    ...
    labels        = unflat_to_c3(label       , num_scales, num_bases, H, W)
    label_assigns = unflat_to_c3(label_assign, num_scales, num_bases, H, W)
    label_weights = unflat_to_c3(label_weight, num_scales, num_bases, H, W)
    ...
    for l in range(num_scales):
        if l==0:
            ...
            pyramid = cv2.resize(image, None, fx=1/2**l,fy=1/2**l)
        ...
        a = cv2.resize(a, None, fx=2**l,fy=2**l,interpolation=cv2.INTER_NEAREST)
        all.append(a)
...
def draw_rpn_target_target(cfg, image, window, target, target_weight):
    ...
    target_weights = unflat_to_c3(target_weight, num_scales, num_bases, H, W)
