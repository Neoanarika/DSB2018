...
def add_truth_box_to_proposal(cfg, proposal, b, truth_box, truth_label):
...
def null():
    ...
    sampled_instance = Variable(torch.LongTensor( 0,1,1)).cuda()
...
def make_one_mask_target(cfg, input, proposal, truth_box, truth_label, truth_instance):
    if len(truth_box)==0 or len(proposal)==0: return null()
    ...
    len_fg_index = len(fg_index)
    fg_index = fg_index[
        np.random.choice(len_fg_index, size=cfg.mask_train_batch_size, replace=len_fg_index<cfg.mask_train_batch_size)
    ]
...
def make_mask_target(cfg, inputs, proposals, truth_boxes, truth_labels, truth_instances):
    ...
    #take care of don't care ground truth. Here, we only ignore them  ----------
