...
from net.resnet50_fpn_mask_single_shot_0.configuration import *


from net.resnet50_fpn_mask_single_shot_0.layer.rpn_multi_nms    import *
from net.resnet50_fpn_mask_single_shot_0.layer.rpn_multi_target import *
from net.resnet50_fpn_mask_single_shot_0.layer.rpn_multi_loss   import *

from net.resnet50_fpn_mask_single_shot_0.layer.mask_nms    import *
from net.resnet50_fpn_mask_single_shot_0.layer.mask_target import *
from net.resnet50_fpn_mask_single_shot_0.layer.mask_loss   import *
...
class FeatureNet(nn.Module):
    ...
    def __init__(self, cfg, in_channels, out_channels=128 ):
        ...
        self.layer_c1 = make_layer_c(   64,  64,   64, num_blocks=2, stride=1)
        self.layer_c2 = make_layer_c(   64,  32,  128, num_blocks=3, stride=2)
        self.layer_c3 = make_layer_c(  128,  64,  256, num_blocks=4, stride=2)
        self.layer_c4 = make_layer_c(  256, 128,  512, num_blocks=6, stride=2)
        self.layer_c5 = make_layer_c(  512, 256, 1024, num_blocks=3, stride=2)

        # top-down
        self.layer_p5 = nn.Conv2d   (1024, out_channels, kernel_size=1, stride=1, padding=0)
        self.layer_p4 = LateralBlock( 512, out_channels, out_channels)
        self.layer_p3 = LateralBlock( 256, out_channels, out_channels)
        self.layer_p2 = LateralBlock( 128, out_channels, out_channels)
        self.layer_p1 = LateralBlock(  64, out_channels, out_channels)
...
class RpnMultiHead(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        for l in range(self.num_scales):
        	self.convs.append ( nn.Conv2d(in_channels, 2*in_channels,                          kernel_size=3, padding=1) )
        	self.logits.append( nn.Conv2d(2*in_channels, self.num_bases[l]*self.num_classes,   kernel_size=3, padding=1) )
        	self.deltas.append( nn.Conv2d(2*in_channels, self.num_bases[l]*self.num_classes*4, kernel_size=3, padding=1) )
    ...
    def __init__(self, cfg, in_channels, out_channels ):
        ...
        for l in range(self.num_scales):
            if self.scales[l] == 1:
                self.convs.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels, out_channels//self.num_scales, kernel_size=3, padding=1),
                ))
            else:
                self.convs.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels, out_channels//self.num_scales, kernel_size=3, padding=1),
                        nn.Upsample(scale_factor=self.scales[l],mode='bilinear'),
                ))
...
class MaskHead(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.logit = nn.Conv2d( 256,num_classes, kernel_size=1, padding=0, stride=1)
...
class MaskSingleShotNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.crop        = CropRoi (cfg, feature_channels, feature_channels*2)
        self.mask_head   = MaskHead(cfg, feature_channels*2)
    ...
    def forward(self, inputs, truth_boxes=None,  truth_labels=None, truth_instances=None ):
        ...
        self.rpn_proposals = rpn_nms(cfg, mode, inputs, self.rpn_logits_flat, self.rpn_deltas_flat, self.rpn_window)
        ...
        if len(self.mask_proposals)==0:
            self.masks = make_empty_masks(cfg, mode, inputs)
        else:
            crops = self.crop(features, self.mask_proposals)
            self.mask_logits = data_parallel(self.mask_head, crops)
            self.masks = mask_nms(cfg, mode, inputs, self.mask_proposals, self.mask_logits) #<todo> better nms for mask
    ...
    def loss(self, inputs, truth_boxes, truth_labels, truth_instances):
        ...
        #self.mask_cls_loss = Variable(torch.zeros((1))).cuda().sum()
        ...
        self.total_loss = self.rpn_cls_loss + self.rpn_reg_loss + 2*self.mask_cls_loss
