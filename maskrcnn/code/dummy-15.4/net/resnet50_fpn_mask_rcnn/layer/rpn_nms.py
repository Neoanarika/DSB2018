...
def make_rpn_windows(cfg, fs):
    ...
    for l in range(num_scales):
        bases   = make_bases(cfg.rpn_base_sizes[l], cfg.rpn_base_apsect_ratios)
...
# this is in gpu
def rpn_nms(cfg, mode, inputs, logits_flat, deltas_flat, windows):
    ...
    probs_flat = F.sigmoid(logits_flat)
    ...
    height, width = inputs.size( )[2:4] #original image width
    ...
    for b in range(batch_size):
        ...
        if nms_min_size != -1:
            keep = torch_filter_boxes(bs, nms_min_size)
            bs = bs[keep]
            ps = ps[keep]

        #pre selection by top N
        #https://github.com/pytorch/pytorch/pull/2329
        idx1 = ps.sort(0, descending=True)[1]
        idx1 = idx1[:nms_pre_top_n]

        #pre selection by greater than threshold
        idx2 = (ps > nms_pre_score_threshold).nonzero().view(-1)

        #Non-maximal suppression
        idx = idx1 if len(idx1)>len(idx2) else idx2
        ps = ps[idx].view(-1, 1)
        bs = bs[idx, :]
        keep = torch_nms(torch.cat((bs, ps), 1), nms_overlap_threshold)
        if nms_post_top_n > 0:
        ...
        proposal[:,0  ]=b
        proposal[:,1:5]=torch.round(bs[keep])
        proposal[:,5  ]=ps[keep]
