...
from net.resnet50_fpn_mask_single_shot.layer.rpn_nms import*
...
def unflat(data, num_sizes, H, W, num_aspect_ratios):

    data = data.reshape( H, W, num_sizes, num_aspect_ratios).transpose([2,0,1,3])
    datas=[]
...
def draw_rpn_target_label(cfg, image, window, label, label_assign, label_weight):
    ...
    labels        = unflat(label       , num_sizes, H, W, num_apsect_ratios)
    label_assigns = unflat(label_assign, num_sizes, H, W, num_apsect_ratios)
    label_weights = unflat(label_weight, num_sizes, H, W, num_apsect_ratios)
    ...
    for l in range(num_sizes):
        a = np.vstack((
            image.copy(),
            labels[l]*np.array([1,1,1]),
            label_assigns[l]*np.array([1,1,1]),
            label_weights[l]*np.array([1,1,1]),
        ))
...
def draw_rpn_target_target(cfg, image, window, target, target_weight):
    ...
    target_weights = unflat(target_weight, num_sizes, H, W, num_apsect_ratios)
    ...
    for l in range(num_sizes):
        a = np.vstack((
            image.copy(),
            target_weights[l]*np.array([1,1,1]),
        ))
...
def make_one_rpn_target(cfg, input, window, truth_box, truth_label):
    ...
    if num_truth_box!=0:
        ...
        target[fg_index] = box_transform(target_window, target_truth_box)
        ...
        if 1:
            ...
            num_sizes         = len(cfg.rpn_base_sizes)
            num_apsect_ratios = len(cfg.rpn_base_apsect_ratios)
            label_weight = label_weight.reshape(height,width,num_sizes,num_apsect_ratios)
            for y, x in itertools.product(range(height),range(width)):
                for l in range(num_sizes):
                    label_weight[y,x,l] *= 1/(2**l)
