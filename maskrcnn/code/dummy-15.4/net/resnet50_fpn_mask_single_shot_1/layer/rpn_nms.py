...
def make_rpn_windows(cfg, f):

    rpn_bases   = []
    for base_size in cfg.rpn_base_sizes:
        bases   = make_bases(base_size, cfg.rpn_base_apsect_ratios)
        rpn_bases.append(bases)
    rpn_bases = np.vstack(rpn_bases)

    rpn_windows = make_windows(f, 1, rpn_bases)
...
# this is in gpu
def rpn_nms(cfg, mode, inputs, probs_flat, deltas_flat, window):
    ...
    probs  = probs_flat.data.cpu().numpy()
    ...
    for b in range(batch_size):
        ...
        #if len(proposal)==0: proposal=None
