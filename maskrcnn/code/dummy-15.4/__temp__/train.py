...
def run_train():
    ...
    out_dir  = RESULTS_DIR + '/mask-rcnn-dummy-2'  #'/mask-rcnn-gray-011a-debug'
    initial_checkpoint = \
        RESULTS_DIR + '/mask-rcnn-dummy-2/checkpoint/00001400_model.pth'
        #None #
    ...
    train_dataset = DummyDataset(  #ScienceDataset(
    ...
    valid_dataset = DummyDataset(  #ScienceDataset(
                                'valid1_ids_gray_only1_43', mode='train',
    ...
    while  i<num_iters:  # loop over the dataset multiple times
        ...
        for inputs, truth_boxes, truth_labels, truth_instances, metas, indices in train_loader:
            ...
            if 0: #<debug>
                ...
                for b in range(batch_size):
                    image = (images[b].transpose((1,2,0))*255).astype(np.uint8)
                    image = np.clip(image.astype(np.float32)+64,0,255)  #improve contrast
                    ...
                    if truth_box is not None and detection is not None:
                        ...
                        detection = detection[detection[:,5]>0.9]
                    ...
                    all = np.hstack([image_truth,image_truth1,image_detection,image_detection1])
                    image_show('all',all,2)
                    cv2.waitKey(0)
                    pass
