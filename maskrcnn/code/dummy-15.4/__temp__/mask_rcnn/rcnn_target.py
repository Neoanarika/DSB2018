...
def make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label):
    ...
    #filter proposal at boundary of image ---------
    ...
    for i in range(num_proposal):
        box = proposal[i,1:5]
        if not( is_small_box_at_boundary(box,width,height, min_size=cfg.rcnn_train_min_size) ):
            valid.append(i)
...
def make_rcnn_target(cfg, inputs, proposals, truth_boxes, truth_labels):
