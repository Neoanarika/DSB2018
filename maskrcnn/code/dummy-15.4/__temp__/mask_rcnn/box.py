...
from model.mask_rcnn_lib.nms.cython_nms import cython_nms

#from net.lib.mask_rcnn.nms.py_nms import py_nms
#
...
def is_small_box_at_boundary(box,W,H, min_size):
    ...
    return (x0==0 or y0==0 or x1==W-1 or y1==H-1) and \
        not(w >=min_size or h>=min_size)
...
def is_small_box(box, min_size):
    ...
    aspect = max(w,h)/min(w,h)
    area = w*h
    return not(w >=min_size or h>=min_size)
